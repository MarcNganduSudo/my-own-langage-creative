...
condition_si:   TOK_SI TOK_PARG expression_booleenne TOK_PARD TOK_DOUBLE_POINT bloc_code{
                    $$=g_node_new((gpointer)SI);
                    g_node_append($$,$3);
                    g_node_append($$,$6);
                };

condition_sinon:   TOK_SINON TOK_DOUBLE_POINT bloc_code{
                        $$=g_node_new((gpointer)SINON);
                        g_node_append($$,$3);
                    };

boucle_for:		TOK_FOR TOK_ACG expression_arithmetique TOK_ACD TOK_DOUBLE_POINT bloc_code TOK_FINSTR{
					$$=g_node_new((gpointer)BOUCLE_FOR);
					g_node_append($$,$3);
                    g_node_append($$,$6);
				};

boucle_while:	TOK_WHILE TOK_PARG expression_booleenne TOK_PARD TOK_DOUBLE_POINT bloc_code TOK_FINSTR{
					$$=g_node_new((gpointer)BOUCLE_WHILE);
					g_node_append($$,$3);
                    g_node_append($$,$6);
				};

boucle_do_while:	TOK_DO_WHILE TOK_DOUBLE_POINT bloc_code  TOK_WHILE_tq TOK_ACG expression_booleenne TOK_ACD TOK_FINSTR{
...
affichage:	TOK_AFFICHER TOK_AFFECT expression_arithmetique TOK_FINSTR{
			// printf("\t\tAffichage de la valeur de l'expression arithmetique\n");
			$$=g_node_new((gpointer)AFFICHAGEE);
			g_node_append($$,$3);
		}
		|
		TOK_AFFICHER TOK_AFFECT expression_booleenne TOK_FINSTR{
			// printf("\t\tAffichage de la valeur de l'expression booleenne\n");
			$$=g_node_new((gpointer)AFFICHAGEB);
			g_node_append($$,$3);
		}
		|
		TOK_AFFICHER TOK_AFFECT expression_texte TOK_FINSTR{
